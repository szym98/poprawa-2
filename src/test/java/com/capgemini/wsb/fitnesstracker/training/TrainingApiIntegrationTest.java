...
class TrainingApiIntegrationTest extends IntegrationTestBase {
    ...
    // Nie wiem dlaczego ten cholerny test wykonuje się pojedynczo, a wywala podczas wykona razem z innymi
    // dla Statistics ten test przechodzi, niezalaznie czy pojedynczo czy grupowo, odpalamy, co za shit...
    // gowniany hazard, nie wiem gdzie go szukac...
    // metoda GET Request URI = /v1/trainings/6
    // zwraca 200 i za razem  brak contentu -> java.lang.AssertionError: No value at JSON path "$[0].user.id"
    @Test
    @Transactional
    ...
    void shouldReturnAllFinishedTrainingsAfterTime_whenGettingAllFinishedTrainingsAfterTime() throws Exception {
        ...
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS+00:00");
        sdf.setTimeZone(java.util.TimeZone.getTimeZone("UTC"));
        mockMvc.perform(get("/v1/trainings/after/{afterTime}", "2024-05-18").contentType(MediaType.APPLICATION_JSON))
                .andDo(log())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isArray())
                .andExpect(jsonPath("$[0].user.id").value(user1.getId()))
    ...
    void getAllTrainingByActivityType_whenGettingAllTrainingByActivityType() throws Exception {
        ...
        mockMvc.perform(get("/v1/trainings/activity").param("activity", "TENNIS").contentType(MediaType.APPLICATION_JSON))
                .andDo(log())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isArray())
                .andExpect(jsonPath("$[0].user.id").value(user1.getId()))
